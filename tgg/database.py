...
class Database:
    ...
    def init_db(self):
        ...
        with self.get_connection() as conn:
            ...
            # Добавляем поле luck в таблицу пользователей (если еще нет)
            try:
                cursor.execute("ALTER TABLE users ADD COLUMN luck INTEGER DEFAULT 0")
            ...
            # Таблица банковских платежей (для обратной совместимости)
            cursor.execute(
                """
            ...
            # Таблица для платежей через HTTP DonationAlerts
            cursor.execute(
                """
                CREATE TABLE IF NOT EXISTS da_http_payments (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    def add_user(
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute(
                    """
                    INSERT INTO users (user_id, username, first_name, last_name, balance, referrer_id, is_admin)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                    (
                        user_id,
                    ...
                    if cursor.fetchone():
                        ...
                        """,
                            (
                                ...
                                100,
                                "referral_bonus",
                                f"Бонус за приглашение пользователя {user_id}",
                            ),
                        )
    ...
    def update_balance(
        self, user_id: int, amount: int, transaction_type: str, description: str = ""
    ) -> bool:
        """
        ...
        try:
            with self.get_connection() as conn:
                ...
                if new_balance < 0:
                    print(
                        f"Ошибка: недостаточно средств. Текущий баланс: {current_balance}, попытка списать: {-amount}"
                    )
                    return False
                ...
                conn.commit()
                print(
                    f"Баланс пользователя {user_id} изменен на {amount}. Новый баланс: {new_balance}"
                )
                return True
        except Exception as e:
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                ...
                SELECT user_id, username, first_name, last_name, balance, 
                       total_games, total_wins, registration_date, last_activity, is_banned, is_admin
                FROM users
    ...
    def get_user_level(self, user_id: int) -> Dict:
        ...
        with self.get_connection() as conn:
            ...
            next_level = (
                self.get_level(current_level + 1) if current_level < 10 else None
            )
    ...
    def upgrade_user_level(self, user_id: int) -> Dict:
        ...
        with self.get_connection() as conn:
            ...
            if not next_level:
                return {
                    "success": False,
                    "message": "Ошибка получения следующего уровня",
                }
            ...
            """,
                (
                    ...
                    -next_level["price"],
                    "level_upgrade",
                    f"Повышение уровня до {next_level['name']}",
                ),
            )
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                ...
                SELECT u.user_id, u.username, u.first_name, u.balance, 
                       ul.current_level, l.level_name, l.luck_multiplier, ul.total_spent
                FROM user_levels ul
    ...
    # === ПЛАТЕЖИ DONATIONALERTS HTTP ===

    def create_http_payment(
        self,
        donation_id: str,
        username: str,
        amount: float,
        ...
        """Создание записи о платеже из HTTP"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
                INSERT INTO da_http_payments (donation_id, username, amount, coins_amount, message)
                VALUES (?, ?, ?, ?, ?)
            """,
                (donation_id, username, amount, coins, message),
            )
            conn.commit()
            return cursor.lastrowid

    def get_http_payment(self, donation_id: str) -> Optional[Dict]:
        """Получение информации о платеже по ID доната"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
                SELECT id, donation_id, username, amount, coins_amount, message, status, user_id, created_at, processed_at, admin_id
                FROM da_http_payments WHERE donation_id = ?
            """,
                (donation_id,),
            )
            row = cursor.fetchone()
    ...
    def confirm_http_payment(
        self, donation_id: str, admin_id: int, user_id: int
    ) -> bool:
        """Подтверждение платежа и начисление монет пользователю"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            ...
            cursor.execute(
                """
                SELECT coins_amount FROM da_http_payments 
            ...
            cursor.execute(
                ...
                SET status = 'completed', processed_at = CURRENT_TIMESTAMP, admin_id = ?, user_id = ?
                WHERE donation_id = ?
            """,
                (admin_id, user_id, donation_id),
            )
            ...
            """,
                (
                    ...
                    coins,
                    "da_http",
                    f"Пополнение через DonationAlerts HTTP: {donation_id}",
                ),
            )
    ...
    def get_pending_http_payments(self) -> List[Dict]:
        """Получение всех ожидающих платежей"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
                SELECT id, donation_id, username, amount, coins_amount, message, created_at
                FROM da_http_payments
    ...
    def get_user_http_payments(self, user_id: int, limit: int = 10) -> List[Dict]:
        """Получение истории HTTP платежей пользователя"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
                SELECT id, donation_id, username, amount, coins_amount, message, status, created_at, processed_at
                FROM da_http_payments
                WHERE user_id = ?
    ...
    def get_http_payment_stats(self) -> Dict:
        """Получение статистики по HTTP платежам"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            ...
            cursor.execute(
                """
                SELECT SUM(amount) FROM da_http_payments WHERE status = 'completed'
            """
            )
            ...
            cursor.execute(
                """
                SELECT COUNT(*) FROM da_http_payments WHERE status = 'completed'
            """
            )
            ...
            cursor.execute(
                """
                SELECT SUM(amount) FROM da_http_payments 
                WHERE status = 'completed' AND DATE(processed_at) = ?
            """,
                (today,),
            )
            ...
            cursor.execute(
                """
                SELECT COUNT(*) FROM da_http_payments WHERE status = 'pending'
            """
            )
    ...
    # === БАНКОВСКИЕ ПЛАТЕЖИ (для обратной совместимости) ===
    ...
    def create_bank_deposit(self, user_id: int, amount: int) -> Dict:
        ...
        expires_at = (
            datetime.datetime.now() + datetime.timedelta(hours=PAYMENT_EXPIRY_HOURS)
        ).isoformat()
        ...
        with self.get_connection() as conn:
            ...
            """,
                (
                    ...
                    coins,
                    "bank_deposit",
                    f"Пополнение через банк #{deposit_id}",
                ),
            )
    ...
    def get_user_payment_history(self, user_id: int, limit: int = 10) -> Dict:
        ...
        with self.get_connection() as conn:
            ...
            # История HTTP пополнений
            cursor.execute(
                """
                SELECT id, amount, coins_amount, donation_id, status, created_at, processed_at
                FROM da_http_payments
                WHERE user_id = ?
                ORDER BY created_at DESC
                LIMIT ?
            ...
            return {
                ...
                "level_upgrades": [
                    {"id": l[0], "amount": l[1], "description": l[2], "date": l[3]}
        ...
        with self.get_connection() as conn:
            ...
            """,
                (
                    ...
                    bonus,
                    "daily_bonus",
                    f"Ежедневный бонус (стрик: {streak} дней)",
                ),
            )
