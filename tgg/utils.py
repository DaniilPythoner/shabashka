...
def play_guess_game(bet: int, guess: int) -> Tuple[int, str]:
    ...
    Выигрыш - это ЧИСТАЯ сумма, которую нужно добавить к балансу
    """
    dice = roll_dice()
    win = dice == guess

    if win:
        win_amount = bet * 5  # Чистый выигрыш (без учета ставки)
        result_text = (
...
def play_highlow_game(bet: int) -> Tuple[int, str]:
    ...
    if dice <= 3:
        ...
        win_amount = bet  # Ничья - возврат ставки
        result_text = (
            f"{dice_emoji} Вам выпало: {dice}\n\n"
...
def play_craps_game(bet: int) -> Tuple[int, str]:
    ...
    if total in [7, 11]:
        win_amount = int(bet * 1.5)  # Выигрыш x1.5
