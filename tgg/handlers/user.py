...
async def cmd_admin(message: types.Message):
    ...
    from handlers.admin import is_admin, get_admin_main_keyboard
...
async def debug_admin(message: types.Message):
    """Отладка системы администраторов"""
    from handlers.admin import is_admin

    user_id = message.from_user.id
    user = db.get_user(user_id)

    text = f"🔍 **Отладка админ-панели**\n\n"
    text += f"Ваш ID: `{user_id}`\n"
    text += f"Есть в config ADMIN_IDS: {'✅' if user_id in ADMIN_IDS else '❌'}\n"
    text += f"ADMIN_IDS список: {ADMIN_IDS}\n\n"

    if user:
        text += f"Запись в БД:\n"
        text += f"  - is_admin: {user.get('is_admin', 0)}\n"
        text += f"  - is_banned: {user.get('is_banned', 0)}\n"
        text += f"  - username: @{user.get('username', 'нет')}\n"
    else:
        text += f"❌ Пользователь не найден в БД!\n"

    text += f"\nФункция is_admin() вернула: {is_admin(user_id)}"
...
async def cmd_myid(message: types.Message):
    """Узнать свой ID и статус администратора"""
    user_id = message.from_user.id
    user = db.get_user(user_id)
...
async def wallet_menu(callback: types.CallbackQuery):
    ...
    text = (
        ...
        f"• Пополнение через банковский перевод\n"
...
async def bank_menu_handler(callback: types.CallbackQuery):
    """Меню банковских операций"""
    from keyboards import get_bank_menu_keyboard

    user_id = callback.from_user.id
    user = db.get_user(user_id)

    text = (
        f"🏦 **Банковские операции**\n\n"
        f"💰 Ваш баланс: **{format_number(user['balance'])}** монет\n\n"
        f"**Доступные операции:**\n"
        f"• Пополнение через банковский перевод\n"
        f"• Вывод средств на банковскую карту\n\n"
    ...
    await callback.message.edit_text(
        text, parse_mode="Markdown", reply_markup=get_bank_menu_keyboard()
    )
    await callback.answer()
...
async def payment_history(callback: types.CallbackQuery):
    ...
    if history["deposits"]:
        text += "**Пополнения:**\n"
        for dep in history["deposits"][:5]:
            status_emoji = (
                "✅"
                if dep["status"] == "completed"
                else "⏳" if dep["status"] == "pending" else "❌"
            )
            text += f"{status_emoji} {dep['amount']} руб. = {dep['coins']} монет ({dep['created'][:16]})\n"
        text += "\n"
    ...
    if history["withdraws"]:
        ...
        for wd in history["withdraws"][:5]:
            status_emoji = (
                ...
                else "⏳" if wd["status"] == "pending" else "❌"
            )
            text += f"{status_emoji} {wd['amount']} руб. на карту *{wd['card']} ({wd['created'][:16]})\n"
        text += "\n"

    if not history["deposits"] and not history["withdraws"]:
...
async def select_game(callback: types.CallbackQuery, state: FSMContext):
    """Выбор игры"""
    from keyboards import get_bet_keyboard
