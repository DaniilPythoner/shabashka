...
async def show_user_stats(callback: types.CallbackQuery):
    ...
    stats_text = (
        ...
        f"• Множитель: x{user_level['luck_multiplier']}\n"
    )
...
async def show_referrals(callback: types.CallbackQuery):
    ...
    if referrals:
        ...
        for i, ref in enumerate(referrals[:5], 1):
            ...
            ref_date = format_time_ago(ref["registration_date"])
            text += f"{i}. {name} - {ref['total_games']} игр ({ref_date})\n"
...
async def games_menu(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"🎰 **Дуэль с ботом** - x2\n"
        f"🎲🎲 **Крэпс** - x1.5\n\n"
        f"✨ Ваш множитель удачи: x{user_level['luck_multiplier']}",
        parse_mode="Markdown",
...
async def play_game(
    ...
    """
    Общая функция для запуска игры с учетом уровня удачи
    """
    ...
    if game_type == "guess" and guess:
        win_amount, result_text = play_guess_game(bet_amount, guess, luck_multiplier)
    elif game_type == "highlow":
        win_amount, result_text = play_highlow_game(bet_amount, luck_multiplier)
    elif game_type == "duel":
        win_amount, result_text = play_duel_game(bet_amount, luck_multiplier)
    elif game_type == "craps":
        win_amount, result_text = play_craps_game(bet_amount, luck_multiplier)
    else:
    ...
    result_text += f"\n🎚️ Ваш уровень: {user_level['level_name']} (x{luck_multiplier})"
...
async def faq_item(callback: types.CallbackQuery):
    ...
    faq_texts = {
        ...
        "levels": (
            ...
            "• 1-3: Бронзовые уровни\n"
            "• 4-6: Серебряные уровни\n"
            "• 7-9: Золотые уровни\n"
            "• 10: Бриллиантовый\n\n"
