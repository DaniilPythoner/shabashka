...
💸 Вывод на карту - вывод средств на банковскую карту
"""
...
async def play_game(
    ...
    1. Ставка списывается 1 раз
    2. При выигрыше - начисляется выигрыш (чистыми)
    3. При ничьей - возвращается ставка
    4. При проигрыше - ничего не начисляется
    """
    ...
    # Получаем баланс после списания ставки
    ...
    # НАЧИСЛЯЕМ ВЫИГРЫШ
    if win_amount > 0:
        # Выигрыш - начисляем чистый выигрыш
        ...
        result = "draw"
        final_balance = current_balance + bet_amount
        win_amount = 0  # Для статистики
    else:
