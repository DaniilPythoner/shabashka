...
try:
    from config import (
        BOT_TOKEN, ADMIN_IDS,
        RUB_TO_COINS, MIN_BANK_DEPOSIT, SUPPORT_CONTACT,
        BANK_NAME, BANK_CARD, BANK_ACCOUNT, BANK_BIK,
        HELP_TEXT, BANK_INFO_TEXT, WITHDRAW_TERMS_TEXT,
        START_BALANCE, MIN_BET, MAX_BET, REFERRAL_BONUS, REFERRAL_BONUS_FRIEND
    )
except ImportError as e:
    logger.error(f"❌ Ошибка импорта config.py: {e}")
    print("\n❌ ОШИБКА: Не удалось импортировать config.py")
    print("Убедитесь, что файл config.py существует и содержит все необходимые настройки")
    sys.exit(1)
...
# Импорт обработчиков
try:
    from handlers import user, admin, bank_payments, admin_bank
    from handlers import admin_game_control, levels, admin_levels, admin_luck
    ...
    logger.info(f"   - bank_payments.py: загружен (банковские платежи)")
    logger.info(f"   - admin_bank.py: загружен (админ банка)")
    logger.info(f"   - admin_game_control.py: загружен (управление играми)")
    logger.info(f"   - levels.py: загружен (система уровней)")
    logger.info(f"   - admin_levels.py: загружен (управление уровнями)")
    logger.info(f"   - admin_luck.py: загружен (управление удачей)")
except ImportError as e:
    ...
    print("  - handlers/bank_payments.py")
    print("  - handlers/admin_bank.py")
    print("  - handlers/admin_game_control.py")
    print("  - handlers/levels.py")
    print("  - handlers/admin_levels.py")
    print("  - handlers/admin_luck.py")
    sys.exit(1)
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                ...
                f"• Банк: {BANK_NAME}\n"
                f"• Карта: `{BANK_CARD}`\n"
                f"• Минимальный платеж: {MIN_BANK_DEPOSIT} руб.\n\n"
                f"🎚️ **Система уровней:**\n"
                f"• Всего уровней: 10\n"
...
async def on_shutdown(bot: Bot):
    ...
    try:
        ...
        if active_games_count > 0:
            logger.warning(f"⚠️ Осталось {active_games_count} активных игр при остановке бота")
    except:
        pass
...
def check_environment():
    ...
    if os.path.exists("handlers"):
        ...
        required_files = [
            "__init__.py", "user.py", "admin.py", 
            "bank_payments.py", "admin_bank.py",
            "admin_game_control.py", "levels.py", 
            "admin_levels.py", "admin_luck.py"
        ]
        missing_files = []
    ...
    if os.path.exists("dice_bot.db"):
        db_size = os.path.getsize("dice_bot.db") / 1024
    ...
    if levels and len(levels) == 10:
        ...
        print(f"  • Максимальный: {levels[-1]['name']} (x{levels[-1]['luck_multiplier']})")
    else:
...
async def main():
    ...
    try:
        ...
        dp.include_router(admin_bank.router)
        dp.include_router(admin_game_control.router)
        dp.include_router(levels.router)
        dp.include_router(admin_levels.router)
        dp.include_router(admin_luck.router)
        ...
        logger.info("   - bank_payments.router: банковские платежи")
        logger.info("   - admin_bank.router: админ банка")
        logger.info("   - admin_game_control.router: управление играми")
        logger.info("   - levels.router: система уровней")
        logger.info("   - admin_levels.router: управление уровнями")
        logger.info("   - admin_luck.router: управление удачей")
        ...
        print("3. Пример правильного токена: 1234567890:ABCdefGHIjklMNOpqrsTUVwxyz1234567890")
