...
def get_bank_menu_keyboard():
    ...
    builder.row(
        InlineKeyboardButton(text="📊 История операций", callback_data="bank_history"),
        width=1,
    )
    builder.row(
...
def get_admin_keyboard():
    ...
    builder.row(
        InlineKeyboardButton(
            text="🏦 Платежи (пополнения)", callback_data="admin_payments"
        ),
        InlineKeyboardButton(
...
def get_admin_payments_keyboard():
    """Клавиатура для администрирования платежей"""
    builder = InlineKeyboardBuilder()
    ...
    builder.row(
        InlineKeyboardButton(
            text="📋 Заявки на вывод", callback_data="admin_pending_withdraws"
        ),
        width=1,
    )
    builder.row(
