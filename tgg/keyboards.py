...
def get_games_keyboard():
    ...
    builder.row(
        InlineKeyboardButton(text="🎯 Больше/Меньше 3 (x2)", callback_data="game_highlow"),
        width=1
    )
    builder.row(
...
def get_bet_keyboard(min_bet: int = 10, max_bet: int = 10000):
    ...
    for bet in bet_options:
        buttons.append(InlineKeyboardButton(text=f"💰 {bet}", callback_data=f"bet_{bet}"))
...
# ============================================
# КЛАВИАТУРЫ ДЛЯ КОШЕЛЬКА
# ============================================

def get_wallet_keyboard():
    """Клавиатура для кошелька (только банковские платежи)"""
    builder = InlineKeyboardBuilder()
    
    builder.row(
        InlineKeyboardButton(text="🏦 Банковский перевод", callback_data="bank_deposit"),
        width=1
    )
    builder.row(
...
def get_withdraw_menu_keyboard():
    ...
    builder.row(
        InlineKeyboardButton(text="💳 Вывести на карту", callback_data="withdraw_request"),
        width=1
    )
    builder.row(
...
def get_payment_history_keyboard():
    ...
    builder.row(
        InlineKeyboardButton(text="📥 Пополнения", callback_data="bank_history"),
        InlineKeyboardButton(text="📤 Выводы", callback_data="withdraw_history"),
        width=2
    )
    builder.row(
        InlineKeyboardButton(text="🔙 Назад", callback_data="wallet_menu"),
...
def get_faq_keyboard():
    """Клавиатура для FAQ (без DonationAlerts)"""
    builder = InlineKeyboardBuilder()
    
    faq_items = [
        ("💰 Как пополнить баланс?", "faq_deposit"),
        ("💸 Как вывести средства?", "faq_withdraw"),
        ("🎲 Как играть?", "faq_games"),
        ("👥 Рефералы", "faq_referrals"),
        ("🎁 Бонусы", "faq_bonus"),
        ("🆔 Мой ID", "faq_myid"),
        ("🎚️ Система уровней", "faq_levels")
    ]
...
# ============================================
# КЛАВИАТУРЫ ДЛЯ УРОВНЕЙ
# ============================================

def get_levels_keyboard():
    """Главная клавиатура для меню уровней"""
    builder = InlineKeyboardBuilder()
...
def get_level_info_keyboard(level_number: int, is_current: bool = False, can_upgrade: bool = False):
    """Клавиатура для информации об уровне"""
    builder = InlineKeyboardBuilder()
    
    if can_upgrade and not is_current:
        builder.row(
            InlineKeyboardButton(text="⬆️ Повысить уровень", callback_data=f"upgrade_from_info_{level_number}"),
            width=1
        )
...
def get_luck_value_keyboard():
    ...
    for val in values:
        buttons.append(InlineKeyboardButton(
            text=f"x{val}", 
            callback_data=f"admin_luck_value_{val}"
        ))
    ...
    builder.row(
        InlineKeyboardButton(text="🔙 Назад", callback_data="admin_luck_menu"),
        width=1
    )
...
def get_admin_level_selection_keyboard(levels: list, action: str, user_id: int):
    """Клавиатура для выбора уровня админом"""
    builder = InlineKeyboardBuilder()
...
def get_admin_level_confirm_keyboard(user_id: int, new_level: int, action: str):
    """Клавиатура подтверждения изменения уровня"""
    builder = InlineKeyboardBuilder()
    
    builder.row(
        InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"admin_level_confirm_{user_id}_{new_level}_{action}"),
        InlineKeyboardButton(text="❌ Отмена", callback_data="admin_levels_menu"),
        width=2
    )
...
def get_url_keyboard(text: str, url: str, back_callback: str = "back_to_main"):
    """Клавиатура с URL-ссылкой и кнопкой назад"""
    builder = InlineKeyboardBuilder()
    builder.row(
