...
def get_wallet_keyboard():
    """Клавиатура для кошелька с DonationAlerts HTTP"""
    builder = InlineKeyboardBuilder()
...
def get_withdraw_amount_keyboard(max_amount: int):
    ...
    builder.row(
        InlineKeyboardButton(text="🔙 Назад", callback_data="withdraw_menu"), width=1
    )
...
def get_admin_pending_http_keyboard(
    payments: list, page: int = 0, total_pages: int = 1
):
    """Клавиатура для списка ожидающих HTTP платежей"""
    builder = InlineKeyboardBuilder()

    # Кнопки для каждого платежа
    for payment in payments[:5]:
        short_id = payment["donation_id"][:8] if payment["donation_id"] else "???"
        builder.row(
            InlineKeyboardButton(
                text=f"💰 {payment['amount']} руб. - {payment['username']}",
                callback_data=f"admin_view_http_{payment['donation_id']}",
            ),
            width=1,
        )
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(text="◀️", callback_data=f"admin_http_page_{page-1}")
        )
...
def get_admin_http_payment_keyboard(donation_id: str):
    ...
    builder.row(
        InlineKeyboardButton(
            text="✅ Подтвердить", callback_data=f"http_bind_{donation_id}"
        ),
        InlineKeyboardButton(
            text="❌ Отклонить", callback_data=f"http_confirm_{donation_id}"
        ),
        width=2,
    )
    builder.row(
...
def get_error_keyboard(back_callback: str = "back_to_main"):
    ...
    builder.row(
        InlineKeyboardButton(text="📧 Поддержка", callback_data="support"), width=1
    )
    return builder.as_markup()
